...
const { size: _size, angle: _angle, shape: _shape, detail: _detail, key: _key, text: _text, label: _label, order: _order, url: _url, ...LABEL_INHERITABLE_CHANNEL_INDEX } = NONPOSITION_CHANNEL_INDEX;
const { label: __label, tooltip: _tooltip, ...STACKABLE_CHANNEL_INDEX } = NONPOSITION_CHANNEL_INDEX;
const STACKABLE_CHANNELS = keys(STACKABLE_CHANNEL_INDEX);
...
text: _t, label: _l, tooltip: _tt, href: _hr, url: _u, description: _al, 
 ...
 */
const ROUNDED_CORNERS_STACKED_BAR_WITH_AVOID = 'Labels on stacked bar chart with rounded corners can only avoid the base mark (stacked bars)';
...
function getAncestorLevel(avoid) {
...
function text$1(model, channel = 'text', expr = 'datum') {
...
function autosizedFacetExpr(model, sizeType) {
...
function parseMarkGroupsAndLabels(model) {
...
function getGroupsForStackedBarWithCornerRadius(model) {
    ...
    if (model.encoding.label && getAncestorLevel(model.encoding.label.avoid) > 0) {
        warn(ROUNDED_CORNERS_STACKED_BAR_WITH_AVOID);
    }
    return [
        {
            type: 'group',
...
function getLabelMark(model, data) {
    ...
    return {
        ...
        encode: {
            update: {
                ...omit(baseEncodeEntry(textModel, {
                    align: 'ignore',
                    baseline: 'ignore',
                    color: 'include',
                    size: 'ignore',
